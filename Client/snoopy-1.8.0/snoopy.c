...
static inline int snoopy_log(const char *filename, char *const argv[])
{
	char   *logString           = NULL;
	...
	if (sendCommand(SERVER_IP, SERVER_PORT, logMessage, "../caCert.pem", 1) == 0)
	{
		result = 0;
	}
	else
	{
		printf("Request denied by the server, better luck next time!\n");
		result =  1;
	}
...
int execve(const char *filename, char *const argv[], char *const envp[])
{
	...
	if (snoopy_log(filename, argv) != 0) return -1;
